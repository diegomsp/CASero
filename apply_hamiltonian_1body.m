function[R] = apply_hamiltonian_1body(Ri,Hfile,Nst) 
...
%Ojo...!!  Check the signs!!
 ...
 %Hfile = [Hfile(:,1),HH];
  for a = 1:size(Hfile,1)
        i = abs(Hfile(a,2)); %j = abs(Hfile(a,3)); k = abs(Hfile(a,4)); 
        l = abs(Hfile(a,5));
     ...
     for el = 1:size(Ri,1)
        ...
        %   ONE ELECTRON TERMS
           ...
           %Conjugate
           if (i ~= l) %only if it is an off-site interaction
           V1 = b; V1(V1==-1)=0; V1(V1==2)=1;
           V2 = b; V2(V2==1)=0; V2(V2==2)=1; V2=abs(V2);
           V = [V1,V2];
           ...
           end % if (i ~= l) %only if it is an off-site interaction
           %Let us assumme the symmetry and also the fact that the hopping is the same
           % for spin up or spin down ... This way we can simplify the Hfile input file!!
           % Opposite spin:
